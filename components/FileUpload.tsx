...
const FileUpload: React.FC<FileUploadProps> = ({ label, onUploaded, accept, fileUrl }) => {
    ...
    useEffect(() => {
        if (fileUrl) {
            ...
            setFilePreview(fileUrl.endsWith('.pdf') ? '/pdf_image.png' : '/file_image.png');
        }
    }, [fileUrl]);

    const handleFile = async (file: File) => {
        if (!file) return;
        if (file.size > 10 * 1024 * 1024) return toast.error('File size should be less than 10MB');
        ...
        try {
            ...
            if (response.type === 'multimedia-upload/image/rejected') throw new Error(response.payload.message);
                ...
                {filePreview && !fileUrl ? (
                    filePreview.startsWith('data:image') ? (
                        ...
                        <div className="flex flex-col items-center">
                            <img src={filePreview} alt="file preview" className="w-12 h-12 mb-2" />
                ...
                <div className="flex items-center justify-between gap-x-2 mt-2">
                    ...
                    <div className="flex items-center gap-x-2">
                        {/\.(jpe?g|png|gif|webp)$/i.test(fileUrl) ? (
                            <img src={fileUrl} alt="file preview" className="size-7 rounded object-cover" />
                        ) : (
                            <img
                                src={fileUrl.endsWith('.pdf') ? '/pdf_image.png' : '/file_image.png'}
                                alt="file icon"
                                className="size-6"
                            />
                        )}
                    ...
                    <a href={fileUrl} target="_blank" rel="noopener noreferrer" className="text-gray-600">
