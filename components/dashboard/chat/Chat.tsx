...
import { messagesRetrieved, messagesSent, retrieveMessages, sendMessage, updateMessagesReadStatus } from "@/redux/slices/chatSlice"
import { useSocket } from "@/context/SocketProvider"
import type { Message, MessagesResponse } from "@/types/chat"
import Link from "next/link"
import { IoIosDocument, IoIosImage, IoIosClose, IoIosDownload } from "react-icons/io"
...
const MessageSkeleton = ({ isOwnMessage }: { isOwnMessage: boolean }) => {
  return (
    <div className={`flex ${isOwnMessage ? "justify-end" : "justify-start"} mb-4`}>
      <div
        className={`max-w-xs md:max-w-md lg:max-w-lg p-3 rounded-2xl ${isOwnMessage
...
export default function Chat({
  ...
  const { messages, chat, latestMessage } = useSelector((state: RootState) => state.chat)
  ...
  useEffect(() => {
    ...
    return () => {
      socketService.off(`messagesRetrieved:${profile?.id}`, handleMessagesRetrieved)
    }
  }, [token, dispatch, profile?.id, chatbuddyId, isConnected, isInitialLoad])
  ...
  useEffect(() => {
    ...
    return () => {
      socketService.off(`messageReceived:${updatedChatId}`, handleIncomingMessage)
  ...
  const renderMessage = (message: Message) => (
    <div key={message.id} className={`flex ${message.initiator_id === profile?.id ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-xs md:max-w-md lg:max-w-lg p-3 rounded-2xl ${message.initiator_id === profile?.id
          ? "bg-indigo-100 dark:bg-primary-main rounded-se-none"
          : "bg-gray-100 dark:bg-gray-700 rounded-ss-none"
          }`}
      >
        {message.file && (
          <div className="mb-2 relative group">
            {message.file.startsWith("data:image/") || message.file.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
              <div
              ...
              >
                <Image src={message.file || "/placeholder.svg"} alt="Uploaded content" fill className="object-cover" />
        ...
        {message.message && <p className="text-sm text-gray-800 dark:text-white font-bold">{message.message}</p>}
        <div className="flex justify-end items-center mt-1 gap-1">
          <span className="text-xs text-gray-500 dark:text-gray-300">{formatMessageTime(message.created_at)}</span>
          {message.initiator_id === profile?.id && (
            <span className={`inline-block w-2 h-2 rounded-full ${message.read ? "text-blue-500" : "text-gray-400"}`}>
              ✓
            </span>
          )}
        </div>
  ...
  return (
    <div className={cn("mx-auto flex flex-col justify-between h-full w-full")}>
      <div className="flex flex-col">
        ...
        <div className="flex items-center gap-3 mb-2 px-2 md:px-3 py-2 border-b dark:border-black-2 bg-neutral-4 dark:bg-gray-800 rounded-xl justify-between">
          <div className="flex gap-2 items-center">
            <div className="flex items-center">
              {enabledBackButton && (
                <Link
                  href={profile?.role.role_id === SystemRole.USER ? "/dashboard/messages" : "/messages"}
            ...
            <div className="flex flex-col">
              <p className="font-semibold text-gray-800 dark:text-white">{chat?.chat_buddy?.name}</p>
            </div>
          </div>
          {rightSideComponent}
        </div>
        ...
        {/* Messages */}
        <div className={cn("flex-1 overflow-y-auto p-4 space-y-6", height && height)} ref={messageContainerRef}>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center h-full min-h-[full] text-center">
              <div className="w-16 h-16 bg-gray-100 dark:bg-gray-700 rounded-full flex items-center justify-center mb-4">
                <Icon url="/icons/chat/chats.svg" width={24} className="text-gray-400" />
              </div>
              <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-2">No messages yet</h3>
              <p className="text-gray-500 dark:text-gray-400 max-w-sm">
                Start a conversation by sending your first message to {chat?.chat_buddy?.name}
              </p>
            </div>
          )}
          ...
          {filePreview && (
            <div className="absolute bottom-full left-0 right-0 mb-2 bg-white dark:bg-gray-800 rounded-lg p-2 shadow-lg border dark:border-gray-700">
              <div className="flex items-center gap-2">
                ...
                {isUploading ? (
                  <button onClick={handleCancelUpload} className="text-red-500 hover:text-red-600 p-1">
                    <IoIosClose className="text-xl" />
                  </button>
                ) : (
                  <button onClick={removeFilePreview} className="text-gray-500 hover:text-red-500 p-1">
                    <IoIosClose className="text-xl" />
                  </button>
                )}
              </div>
